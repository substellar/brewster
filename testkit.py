...
def lnprior(theta,obspec):
    ...
    if (all(invmr[0:5] > -12.0) and (np.sum(10**(invmr[0:5])) < 1.0) and ((0.001*np.min(obspec[2,:]**2)) < 10.**logf < (100.*np.max(obspec[2,:]**2))) and  (min(T) > 10.0) and (max(T) < 4000) and (gam > 0.) and (-5. < logbeta < 0.)):
    	beta=10.**logbeta
    	alpha=1.0
    	x=gam
