...
def lnprob(theta,pcover, cloudtype, do_clouds,gasnum,cloudnum,inlinetemps,coarsePress,press,inwavenum,linelist,cia,ciatemps,use_disort,fwhm,obspec):
    invmr = theta[0:4]
    # This is a fudge to make 1 patch 1 cloud work
    cloudparams = theta[4:9]
    logg = theta[9]
    r2d2 = theta[10]
    dlam = theta[11]
    gam = theta[12]
...
def lnprior(theta,obspec):
    ...
    #         and  and (-5. < logbeta < 0))
    if (all(invmr[0:4] > -12.0) and (np.sum(10.**(invmr[0:4])) < 1.0) 
        and  0.0 < logg < 6.0
        and cloudparams[0] > 0.0
        and 1e-2  < cloudparams[1] < 2e5
        and cloudparams[2] >  cloudparams[1]
        and 0. < cloudparams[3] < 1.0
        and -1.0 < cloudparams[4] < +1.0
#        and 1. < M < 80. 
        and  0.5 < r2d2 < 1.5 
        and -0.01 < dlam < 0.01 
