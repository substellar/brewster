...
for result in sampler.sample(pos, iterations=60000):
	...
	output=[chain,lnprob]
	pickle.dump(output,open("/nobackup/bburning/MCMC_pow.pic","wb"))
	pickle.dump(chain[:,k-1,:], open('MCMC_last_pow.pic','wb'))


chain=sampler.chain
lnprob=sampler.lnprobability
output=[chain,lnprob]
pickle.dump(output,open("/nobackup/bburning/2m2224_pow_cloud_newTP.pic","wb"))
pickle.dump(chain[:,-1,:], open('MCMC_last_pow.pic','wb'))
...
save_object(sampler,"/nobackup/bburning/2M2224_powcloud_newTP.pk1")
