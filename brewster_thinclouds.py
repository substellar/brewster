...
finalout = "2m0355_2c.pk1"


# periodic dumps/snapshots
# just the chain
chaindump = "2m0355_last_2c.pic"
# The whole thing w/ probs
picdump = "2m0355_snapshot_2c.pic"
...
if (fresh == 0):
    ...
    p0[:,5] = np.random.rand(nwalkers).reshape(nwalkers) + 3.0
    p0[:,6] =  1.0e-19 + np.random.rand(nwalkers).reshape(nwalkers) * 5.e-21
    ...
    # some cloud bits now.
    # Cloud 1 (the high one)
    p0[:,9] = np.random.rand(nwalkers).reshape(nwalkers) 
    p0[:,10] = np.random.randn(nwalkers).reshape(nwalkers) 
    p0[:,11] = np.random.rand(nwalkers).reshape(nwalkers)
    p0[:,12] = np.random.randn(nwalkers).reshape(nwalkers)
    # cloud 2 (the deck)
