...
if (fresh == 0):
    ...
    p0[:,7] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 5.5 # Na+K+Cs
    ...
    p0[:,12] = -15.0 + np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,13] = -4. + 6.*np.random.rand(nwalkers).reshape(nwalkers) 
    p0[:,14] = np.random.rand(nwalkers).reshape(nwalkers) 
    p0[:,15] = 0.5 + 0.1*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,16] = 0.1 + 0.01* np.random.randn(nwalkers).reshape(nwalkers)
...
k=0
times = open("runtimes_mie_Mg2SiO4.dat","w")
times.close()
pos,prob,state = sampler.run_mcmc(p0,20000)
sampler.reset()
for result in sampler.sample(pos, iterations=60000):
    clock[k] = time.clock()
    if (k > 1):
        tcycle = clock[k] - clock[k-1]
        times = open("runtimes_mie_Mg2SiO4.dat","a")
        times.write("*****TIME FOR CYCLE*****")
        times.write(str(tcycle))
        times.close()
    k=k+1
    position = result[0]
    f = open("status_ball_mie_Mg2SiO4.txt", "w")
    f.write("****Iteration*****")
    f.write(str(k))
	...
	output=[chain,lnprob]
	pickle.dump(output,open("/nobackup/bburning/MCMC_mie_Mg2SiO4.pic","wb"))
	pickle.dump(chain[:,k-1,:], open('MCMC_last_mie_Mg2SiO4.pic','wb'))


chain=sampler.chain
lnprob=sampler.lnprobability
output=[chain,lnprob]
pickle.dump(output,open("/nobackup/bburning/2m2224_mie_Mg2SiO4.pic","wb"))
pickle.dump(chain[:,-1,:], open('MCMC_last_mie_MgSiO4.pic','wb'))
...
save_object(sampler,"/nobackup/bburning/2M2224_mie_Mg2SiO4.pk1")
