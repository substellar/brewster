...
k=0
times = open("runtimes_mie_Fe.dat","w")
times.close()
...
for result in sampler.sample(pos, iterations=60000):
    ...
    if (k > 1):
        ...
        times = open("runtimes_mie_Fe.dat","a")
        times.write("*****TIME FOR CYCLE*****")
        times.write(str(tcycle))
        times.close()
    k=k+1
    position = result[0]
    f = open("status_ball_mie_Fe.txt", "w")
    f.write("****Iteration*****")
    f.write(str(k))
	...
	output=[chain,lnprob]
	pickle.dump(output,open("/nobackup/bburning/MCMC_mie_Fe.pic","wb"))
	pickle.dump(chain[:,k-1,:], open('MCMC_last_mie_Fe.pic','wb'))


chain=sampler.chain
lnprob=sampler.lnprobability
output=[chain,lnprob]
pickle.dump(output,open("/nobackup/bburning/2m2224_mie_Fe.pic","wb"))
pickle.dump(chain[:,-1,:], open('MCMC_last_mie_Fe.pic','wb'))
...
save_object(sampler,"/nobackup/bburning/2M2224_mie_Fe.pk1")
