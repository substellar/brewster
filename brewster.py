...
if (fresh == 0):
    ...
    p0[:,2] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 8.0 # CO
    ...
    p0[:,17] = 0.6+ 0.1*np.random.rand(nwalkers).reshape(nwalkers)
    p0[:,18] = 0.1 +0.05*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,19] = 0.2+ 0.05*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,20] = 2.+ 0.2*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,21] = 4000. + (1000.*  np.random.rand(nwalkers).reshape(nwalkers))
    ...
    for i in range (0,nwalkers):
        while True:
            ...
            if (min(Tcheck) > 1.0):
                ...
                p0[i,17] = 0.6+ 0.1*np.random.rand()
                p0[i,18] = 0.1+ 0.05*np.random.randn()
                p0[i,19] = 0.2+ 0.05*np.random.randn()
                p0[i,20] = 2. + 0.2*np.random.randn()
                p0[i,21] = 4000. + (1000.*  np.random.rand())
...
save_object(sampler,'/nobackup/bburning/2M2224_thincloud_result.pk1')
