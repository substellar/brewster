...
finalout = "2m0355_nc.pk1"


# periodic dumps/snapshots
# just the chain
chaindump = "2m0355_last_nc.pic"
# The whole thing w/ probs
picdump = "2m0355_snapshot_nc.pic"

# Names for status file runtimes
statfile = "status_ball_nc.txt"
rfile = "runtimes_nc.dat"
...
if (fresh == 0):
    ...
    p0[:,16] = -1.2 + 0.2*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,17] = 2.25+ 0.2*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,18] = 4200. + (500.*  np.random.randn(nwalkers).reshape(nwalkers))
